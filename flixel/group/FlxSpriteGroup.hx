...
class FlxTypedSpriteGroup<T:FlxSprite> extends FlxSprite
{
	...
	override public function overlapsPoint(point:FlxPoint, InScreenSpace:Bool = false, ?Camera:FlxCamera):Bool
	{
		for (sprite in _sprites)
		{
			if (sprite != null && sprite.exists && sprite.visible && sprite.overlapsPoint(point, InScreenSpace, Camera))
			{
				return true;
			}
		}
	...
	override public function pixelsOverlapPoint(point:FlxPoint, Mask:Int = 0xFF, ?Camera:FlxCamera):Bool
	{
		for (sprite in _sprites)
		{
			if (sprite != null && sprite.exists && sprite.visible && sprite.pixelsOverlapPoint(point, Mask, Camera))
			{
				return true;
			}
		}
	...
	/**
	 * Destroys and removes all instances of `FlxSprite` from the list.
	 */
